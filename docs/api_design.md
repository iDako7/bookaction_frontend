...
- **The SQL was executed after **`docker-compose up -d` started Postgres and the `prisma/seed/module1.json` content was synced into `Quiz.correct_option_index`, ensuring every payload mirrors what Prisma Studio displays on [http://localhost:5555](http://localhost:5555).
