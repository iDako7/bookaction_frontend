...
## Tasks
- [ ] 1.0 Project setup & scaffolding (verify: app boots, base layout visible)
  - [ ] 1.1 Initialize Next.js (App Router) with TypeScript, Tailwind, Shadcn UI base if not already present.
  - [ ] 1.2 Add core dependencies: `@tanstack/react-query`, `zustand`, `axios`, `react-hook-form`, `@hookform/resolvers`, `zod`, `clsx`, `tailwind-merge`, `class-variance-authority`.
  - [ ] 1.3 Configure global layout with Inter font, base styles, and shared header/nav shell.
  - [ ] 1.4 Set up React Query provider and Zustand persistence middleware.

- [ ] 2.0 Mock data provider & API toggle (verify: mock endpoints return data in UI via React Query)
  - [ ] 2.1 Map `docs/course_content.json` into mock endpoints matching API shapes (overview, theme, tutorial, quiz, summary, reflection).
  - [ ] 2.2 Create Axios client + provider switch (mock vs API) configurable via env or flag.
  - [ ] 2.3 Implement React Query hooks for modules, concept tutorial, quiz, summary, and reflection using the toggle.

- [ ] 3.0 Progress state & gating utilities (verify: locks/unlocks behave correctly on refresh)
  - [ ] 3.1 Define Zustand store for per-module/concept progress (intro/practice/summary/reflection flags) with LocalStorage persistence.
  - [ ] 3.2 Implement helpers to compute lock/unlock status using `order_index` and completion state.
  - [ ] 3.3 Add navigation guards/redirections to keep users in correct sequence; allow repeats of completed steps.
