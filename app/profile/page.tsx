...
export default function ProfilePage() {
  ...
  const getConceptCompletionPercentage = useProgressStore((state) => state.getConceptCompletionPercentage);
  ...
  modules.forEach((m) => {
    ...
    if (progress) {
      ...
      const completedInModule = Object.values(progress.concepts).filter(c => c.summaryViewed).length;
  ...
  const trophies = [
    {
      ...
      description: "Mastered the art of active listening and meaningful conversation",
      moduleId: modules[0]?.id, // Link to first module
      icon: <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white">🎧</div>,
    ...
    },
    {
      ...
      icon: <div className="w-10 h-10 bg-indigo-500 rounded-full flex items-center justify-center text-white">🗣️</div>,
    ...
    <div className="min-h-screen bg-blue-50/50 p-6 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card className="bg-white border-blue-100 shadow-sm overflow-hidden rounded-2xl p-8 flex flex-col items-center text-center relative">
          ...
          <p className="text-slate-600 text-lg mt-2">Keep building your relationship skills!</p>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="p-6 flex flex-col items-center justify-center text-center border-blue-100 shadow-sm">
            ...
            <div className="text-4xl font-bold text-slate-900">{completedConcepts}</div>
          ...
          <Card className="p-6 flex flex-col items-center justify-center text-center border-blue-100 shadow-sm">
            ...
            <div className="text-4xl font-bold text-slate-900">{overallProgress}%</div>
        ...
        <Card className="p-8 border-blue-100 shadow-sm">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-slate-500">{completedConcepts} of {totalConcepts} lessons completed</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-2xl p-6 flex gap-4 items-center border border-indigo-100">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm">
                You&apos;re making great progress! {totalConcepts - completedConcepts} more lessons to go.
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-8 border-blue-100 shadow-sm">
          <div className="flex items-start gap-4 mb-8">
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-900">Trophy Collection</h3>
              <p className="text-slate-500">{earnedTrophiesCount} of {trophies.length} trophies earned</p>
            </div>
          </div>
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  <div className={cn(
                    ...
                    {trophy.earned ? trophy.icon : <Lock className="w-8 h-8 text-slate-400" />}
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900">{trophy.earned ? trophy.title : "???"}</h4>
                    {trophy.earned && (
                    ...
                    <div className="flex justify-between text-xs text-slate-500 mb-1">
                      ...
                      <span>
                        ...
                        {getModuleProgress(trophy.moduleId!) 
                          ? Object.values(getModuleProgress(trophy.moduleId!)!.concepts).filter(c => c.summaryViewed).length
                          : 0} 
                        {" "}/ {modules.find(m => m.id === trophy.moduleId)?.concepts.length || 3} lessons
                      </span>
                    </div>
                    <Progress value={
                      getModuleProgress(trophy.moduleId!) 
                        ? (Object.values(getModuleProgress(trophy.moduleId!)!.concepts).filter(c => c.summaryViewed).length / (modules.find(m => m.id === trophy.moduleId)?.concepts.length || 1)) * 100
                        : 0
