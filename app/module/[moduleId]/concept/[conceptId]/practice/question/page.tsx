...
import { useConceptQuiz, useSubmitQuizAnswer, useUpdateConceptProgress } from "@/lib/hooks/useApi";
...
export default function PracticeQuestionPage({ params }: PageProps) {
  ...
  const submitAnswer = useSubmitQuizAnswer(numericConceptId); // Note: hook might need updating to take quizId, or we pass it to mutate
  
  const saveQuizAnswer = useProgressStore((state) => state.saveQuizAnswer);
  const markPracticeCompleted = useProgressStore((state) => state.markConceptPracticeCompleted);
  
  // Local state
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedIndices, setSelectedIndices] = useState<number[]>([]);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [feedback, setFeedback] = useState<{ correct: boolean; correctIndices: number[]; explanation: string } | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    // In a real app, we'd hit the API. For mock, we can calculate locally or use the mock API response.
    // The mock API `submitQuizAnswer` returns correct indices.
    // We'll simulate the API call structure here.
    
    // Optimistic check for immediate feedback in this mock version
    // (The real API call would happen here)
    ...
    // Save to store
    // We use a dummy quizId (orderIndex) since we don't have stable DB IDs in the mock JSON sometimes
    // In real app, `currentQuestion` would have a unique `id`.
    saveQuizAnswer(numericModuleId, numericConceptId, currentQuestion.orderIndex, selectedIndices);
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h2 className="font-semibold text-slate-900">Question {currentQuestionIndex + 1} of {questions.length}</h2>
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            <div>
              ...
              {currentQuestion.questionType === "multiple_choice" && (
                <Badge variant="secondary" className="mt-3 bg-indigo-50 text-indigo-600 hover:bg-indigo-50">
            ...
            <div className="space-y-3">
              {currentQuestion.options.map((option, index) => {
                ...
                let stateStyles = "border-slate-200 hover:border-blue-300 hover:bg-blue-50/30";
                let icon = currentQuestion.questionType === "single_choice" 
                  ? <Circle className="w-5 h-5 text-slate-300" /> 
                  : <Square className="w-5 h-5 text-slate-300" />;

                if (isSelected && !isSubmitted) {
                  stateStyles = "border-blue-500 bg-blue-50 ring-1 ring-blue-500";
                  icon = currentQuestion.questionType === "single_choice" 
                  ...
                  >
                    <div className="shrink-0">
                      <div className={cn(
                        ...
                        isSelected && !isSubmitted ? "bg-blue-500 text-white border-blue-500" : "bg-white border-slate-300 text-slate-500",
                        isSubmitted && feedback?.correctIndices.includes(index) && "bg-green-500 text-white border-green-500",
                        isSubmitted && isSelected && !feedback?.correctIndices.includes(index) && "bg-red-500 text-white border-red-500"
                      )}>
                        {String.fromCharCode(65 + index)}
                      </div>
                    </div>
                    <span className="font-medium text-slate-700 text-lg flex-1">{option}</span>
                    <div className="shrink-0">
                       {/* Optional end icon */}
                       {isSubmitted && feedback?.correctIndices.includes(index) && <CheckCircle2 className="w-6 h-6 text-green-500" />}
                       {isSubmitted && isSelected && !feedback?.correctIndices.includes(index) && <XCircle className="w-6 h-6 text-red-500" />}
                    </div>
                  </button>
                );
              })}
